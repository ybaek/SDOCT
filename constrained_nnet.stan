...
model {
    ...
    matrix[Q, Q] Cy;
    {
        vector[Nknots1] latent_mean1;
        vector[Nknots2] latent_mean2;
        ...
        latent_mean1 = beta1 * tanh((biases1 + weights1 * xi) / alpha1);
        latent_mean2 = beta2 * tanh((biases2 + weights2 * xi) / alpha2);
        fz = intercept1 + K1 * latent_mean1;
        fy = intercept2 + K2 * latent_mean2;
    }
    // Data conditional likelihood is independent with residual noise
    for (n in 1:N) {
        // Cz[n,n] += square(sigma1);
        // Cy[n,n] += square(sigma2);
